...
def read_data(filename):
  ...
  x_array = np.empty([0], dtype=int)
  y_array = np.empty([0], dtype=int)
  z_array = np.empty([0], dtype=int)
  with open('accelerometer_data.csv') as csvfile:
    reader = csv.DictReader(csvfile, delimiter=";")
    for line in reader:
      timestamps.append(int(line[0]))
      x_array.append(int(line[1]))
      y_array.append(int(line[2]))
      z_array.append(int(line[3]))
  print(len(timestamps), len(x_array), len(y_array), len(z_array))
  print(df)
